...
async function loadReport(): Promise<ScanReport> {
  // In a real run, VIBECHECK_REPORT_PATH points at the .vibecheck/<ts>/report.json
  // the orchestrator just wrote. Falls back to the bundled sample for the demo.
  const reportPath = process.env.VIBECHECK_REPORT_PATH;
  const filePath = reportPath ?? path.join(process.cwd(), "public", "sample-report.json");
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-6 mb-8">
            ...
            <div>
              ...
              <div className="space-y-3">
                {viewports.map((vp) => {
                  const flag = report.visualIssues.find(
                    (vi: any) => vi.viewport === vp.name && vi.route === vp.route
                  ) as any;
                  return (
                    <ViewportCard
                      key={vp.name}
